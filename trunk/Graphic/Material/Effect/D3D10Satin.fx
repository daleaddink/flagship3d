...
float4 MainPS( float3 normal: TEXCOORD0, float3 tangent: TEXCOORD1, float3 binormal: TEXCOORD2, float3 viewVec: TEXCOORD3, float3 texCoord: TEXCOORD4 ) : SV_Target
{
   viewVec = normalize( viewVec );

   // Grab an angle to rotate the tangent around the normal
   // This is done to create a slightly bumpy feeling
   float angle = noiseScale * ( g_NoiseTexture.Sample( Noise, texCoord ) - 0.5 );
