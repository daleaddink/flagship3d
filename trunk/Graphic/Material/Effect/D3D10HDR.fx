// Flagship Effect File : D3D10HDR.fx
// Remark : HDR effect
...
struct QuadVS_Output
{
    ...
    float2 vScreenPosition : TEXCOORD0;
};
...
QuadVS_Output QuadVS( QuadVS_Input Input )
{
    ...
    Output.vScreenPosition = Input.Tex;
    return Output;
}
    ...
    ) : SV_Target
{
    ...
    if( g_bEnableTexture )
        vIntensity *= g_Texture0.Sample(LinerSampler, vTexture);
    ...
    ) : SV_Target
{
    ...
    for( int y = -1; y <= 1; y++ )
    {
        for( int x = -1; x <= 1; x++ )
        {
            vSample = g_Texture0.Sample( PointSampler, In.vScreenPosition, int2(x,y) );
    ...
    ) : SV_Target
{
    ...
    for( int y = -1; y <= 2; y++ )
    {
        for( int x = -1; x <= 2; x++ )
        {
            fResampleSum += g_Texture0.Sample(PointSampler, In.vScreenPosition, int2(x, y));
        }
    }
...
{
    float4 fAdaptedLum = g_Texture0.Sample(PointSampler, float2(0.0f, 0.0f));
    float4 fCurrentLum = g_Texture1.Sample(PointSampler, float2(0.0f, 0.0f));
    
    // The user's adapted luminance level is simulated by closing the gap between
    // adapted luminance and current luminance by 2% every frame, based on a
    // 30 fps rate. This is not an accurate model of human adaptation, which can
    // take longer than half an hour.
    float fNewAdaptation = fAdaptedLum.x + (fCurrentLum.x - fAdaptedLum.x) * ( 1.0f - pow( 0.98f, 30.0f * g_fElapsedTime ) );
...
{
    float4 vSample = g_Texture0.Sample(PointSampler, In.vScreenPosition);
    float4 vBloom = g_Texture1.Sample(LinerSampler, In.vScreenPosition);
    float4 vStar = g_Texture2.Sample(LinerSampler, In.vScreenPosition);
	float4 fAdaptedLum = g_Texture3.Sample(PointSampler, float2(0.0f, 0.0f));
    ...
    if( g_bEnableToneMap )
    {
		vSample.rgb *= g_fMiddleGray/(fAdaptedLum.r + 0.001f);
    ...
    return float4(vSample.rgb, 1.0f);
}
    ...
    ) : SV_Target
{
    ...
    for( int y = -1; y <= 2; y++ )
    {
        for( int x = -1; x <= 2; x++ )
        {
            sample += g_Texture0.Sample(PointSampler, In.vScreenPosition, int2(x, y));
        }
    }
    ...
    ) : SV_Target
{
	...
	for( int i=0; i < 12; i++ )
	{
		sample += g_avSampleWeights[i] * g_Texture0.Sample( PointSampler, In.vScreenPosition + g_avSampleOffsets[i].xy );
	}
...
{
	float4 vSample = g_Texture0.Sample(PointSampler, In.vScreenPosition);
	float  fAdaptedLum = g_Texture1.Sample(PointSampler, float2(0.0f, 0.0f));
    ...
    ) : SV_Target
{
    ...
    {
        vSamplePosition = In.vScreenPosition + g_avSampleOffsets[iSample].xy;
        vColor = g_Texture0.Sample(PointSampler, vSamplePosition);
    ...
    ) : SV_Target
{
    ...
    {
        vSamplePosition = In.vScreenPosition + g_avSampleOffsets[iSample].xy;
        vSample = g_Texture0.Sample(LinerSampler, vSamplePosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[0] * g_Texture0.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[0] * g_Texture0.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[1] * g_Texture1.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[0] * g_Texture0.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[1] * g_Texture1.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[2] * g_Texture2.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[1] * g_Texture1.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[2] * g_Texture2.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[3] * g_Texture3.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[2] * g_Texture2.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[3] * g_Texture3.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[4] * g_Texture4.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[3] * g_Texture3.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[4] * g_Texture4.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[5] * g_Texture5.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[4] * g_Texture4.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[5] * g_Texture5.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[6] * g_Texture6.Sample(LinerSampler, In.vScreenPosition);
	...
	) : SV_Target
{
	...
	vColor += g_avSampleWeights[5] * g_Texture5.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[6] * g_Texture6.Sample(LinerSampler, In.vScreenPosition);
	vColor += g_avSampleWeights[7] * g_Texture7.Sample(LinerSampler, In.vScreenPosition);
...
technique10 RenderScene
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, TransformScene() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, PointLight() ) );
...
technique10 Bloom
{
    pass P0
    {
        ...
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, BloomPS() ) );
...
technique10 Star
{
    ...
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, StarPS() ) );
...
//       points.
//-----------------------------------------------------------------------------
technique10 SampleAvgLum
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, SampleLumInitial() ) );
...
//-----------------------------------------------------------------------------
technique10 ResampleAvgLum
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, SampleLumIterative() ) );
...
//       the evalutation
//-----------------------------------------------------------------------------
technique10 ResampleAvgLumExp
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, SampleLumFinal() ) );
...
//-----------------------------------------------------------------------------
technique10 CalculateAdaptedLum
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, CalculateAdaptedLumPS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_1
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_1PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_2
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_2PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_3
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_3PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_4
{
    pass P0
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_4PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_5
{
    pass P0
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_5PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_6
{
    pass P0
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_6PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_7
{
    pass P0
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_7PS() ) );
...
// Desc: Return the average of N input textures
//-----------------------------------------------------------------------------
technique10 MergeTextures_8
{
    pass P0
    {        
        SetVertexShader( CompileShader( vs_4_0, QuadVS() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, MergeTextures_8PS() ) );
